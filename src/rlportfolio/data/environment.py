...
class Environment:
    ...
    def get_data(self, start_time, end_time, features, window_length, market, codes):
        ...
        for asset in codes:
            ...
            asset_dict[str(asset)]["close"] = (
                asset_dict[str(asset)]["close"] / base_price
            )

            if "high" in features:
                asset_dict[str(asset)]["high"] = (
                    asset_dict[str(asset)]["high"] / base_price
                )

            if "low" in features:
                asset_dict[str(asset)]["low"] = (
                    asset_dict[str(asset)]["low"] / base_price
                )

            if "open" in features:
                asset_dict[str(asset)]["open"] = (
                    asset_dict[str(asset)]["open"] / base_price
                )
    ...
    def step(self, w1, w2, noise):
        if self.FLAG:
            ...
            if noise == "True":
                price = price + np.stack(
                    np.random.normal(0, 0.002, (1, len(price))), axis=1
                )
