...
def res_block(x, trainable):
    x_shortcut = x
    conv1_W = tf.Variable(
        tf.random_normal([1, 1, 32, 32], stddev=0.15), trainable=trainable
    )
...
def build_predictor(inputs, predictor, nes_num, scope, trainable):
    with tf.name_scope(scope):
        if predictor == "CNN":
            ...
            conv1_W = tf.Variable(
                tf.random.truncated_normal([1, L, N, 32], stddev=0.15),
                trainable=trainable,
            )
            layer = tf.nn.conv2d(
                inputs, filter=conv1_W, padding="VALID", strides=[1, 1, 1, 1]
            )
            norm1 = tf.layers.batch_normalization(layer)
            x = tf.nn.relu(norm1)

            conv3_W = tf.Variable(
                tf.random_normal([1, 1, 32, 1], stddev=0.15), trainable=trainable
            )
            conv3 = tf.nn.conv2d(
                x, filter=conv3_W, strides=[1, 1, 1, 1], padding="VALID"
            )
...
class StockActor:
    def __init__(self, sess, predictor, M, L, N):
        ...
        ) = self.build_actor(predictor, self.scopes[1], False)
    ...
    def init_op(self):
        ...
        self.update = [
            tf.assign(t_a, (1 - self.tau) * t_a + self.tau * p_a) for p_a, t_a in params
        ]
    ...
    def build_actor(self, predictor, scope, trainable):
        with tf.name_scope(scope):
            ...
            out = tf.layers.dense(
                net, self.M, activation=tf.nn.softmax, kernel_initializer=w_init
            )
...
class StockCritic:
    def __init__(self, sess, predictor, M, L, N):
        ...
        (
            ...
            self.target_out,
            self.target_previous_action,
        ) = self.build_critic(predictor, self.scopes[1], False)
    ...
    def predict(self, inputs, actions):
        return self.sess.run(
            self.out, feed_dict={self.inputs: inputs, self.actions: actions}
        )
...
class DDPG:
    def __init__(self, predictor, M, L, N, name, load_weights, trainable):
        ...
        ## logpath
        self.logpath = "c:/temp/logdir/" + name
        ...
        if load_weights == "True":
            ...
            try:
                checkpoint = tf.train.get_checkpoint_state(
                    self.logpath + "/saved_network/DDPG"
                )
        ...
        if trainable == "True":
            ...
            self.summary_writer = tf.summary.FileWriter(
                self.logpath + "/summary/DDPG", self.sesson.graph
            )
    ...
    def train(self, method, epoch):
        ...
        critic_loss, q_value = self.critic.train(
            s, a, np.reshape(y_i, (-1, 1)), a_previous
        )
    ...
    def save_model(self, epoch):
        self.saver.save(
            self.sesson,
            self.logpath + "/saved_network/DDPG/" + self.name,
            global_step=epoch,
        )
